...
interface CurrencyContextType {
    tickets: number
    hearts: number
    lastHeartRegen: Date | null
    loading: boolean
    error: string | null
    spendTickets: (amount: number, jokerType: string, gameId?: string) => Promise<boolean>
    spendHeart: () => Promise<boolean>
    regenerateHearts: () => Promise<void>
    addTickets: (amount: number, source?: string) => Promise<any>
    buyHearts: () => Promise<boolean>
    rewardAd: (rewardType: 'hearts' | 'tickets') => Promise<boolean>
    fetchCurrency: () => Promise<void>
    getTimeUntilNextHeart: () => string | null
}
...
export function CurrencyProvider({ children }: { children: ReactNode }) {
    ...
    const spendTickets = async (amount: number, jokerType: string, gameId?: string) => {
        ...
        try {
            ...
            if (data) {
                ...
                setTickets(prev => Math.max(0, prev - amount))
    ...
    const buyHearts = async () => {
        ...
        try {
            // Spend tickets
            // Note: spendTickets updates local state, so we don't need to manually verify inside here if we call it
