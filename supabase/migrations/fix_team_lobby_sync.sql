-- Add waiting_room_id to tournament_lobbies to sync team members
ALTER TABLE tournament_lobbies
ADD COLUMN IF NOT EXISTS waiting_room_id UUID REFERENCES tournament_waiting_rooms(id);

-- Update find_tournament_match to update lobby status and waiting_room_id
...
BEGIN
  ...
  IF v_is_team THEN
    ...
    -- CRITICAL: Update Lobby Status and WaitingRoom ID
...
-- Function to cancel search also needs to clear waiting_room_id
