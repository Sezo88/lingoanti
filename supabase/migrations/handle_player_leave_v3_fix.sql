-- LEAVE ROOM LOGIC V3 (Fix UUID Error & Last Man Standing)
-- Fixed: Removed usage of MAX(uuid) which caused 'function does not exist' error.
-- Logic: Marks 'left' -> Counts active -> If 1 remains, ends game -> Else skips turn if needed.
...
BEGIN
    ...
    -- Fixed: Avoid MAX(uuid) by doing count first
    ...
    IF v_active_count = 1 AND v_room.status = 'playing' THEN
        ...
        -- Get the single survivor (LIMIT 1 is safe here since we know count is 1)
        SELECT user_id INTO v_survivor_id
        FROM room_participants
        WHERE room_id = p_room_id 
        AND status IN ('active', 'playing', 'ready', 'connected')
        LIMIT 1;
