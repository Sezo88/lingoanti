...
-- MATCHING FRONTEND COMMIT 2d3ffcb (uses 'guesses' and 'results' keys)
...
BEGIN
    ...
    -- Add guess to shared state (using 'guesses' and 'results' keys used by old frontend)
    UPDATE rooms SET
        game_state = jsonb_set(
            jsonb_set(
                game_state,
                '{guesses}',
                COALESCE(game_state->'guesses', '[]'::jsonb) || to_jsonb(p_guess)
            ),
            '{results}',
            COALESCE(game_state->'results', '[]'::jsonb) || to_jsonb(p_result)
        )
    WHERE id = p_room_id;
