...
DECLARE
    ...
    v_initial_word TEXT;
BEGIN
    -- Ensure game_mode column exists on games table
    ...
    IF v_queue_record.user_id IS NOT NULL THEN
        ...
        -- CRITICAL: Set game_mode='quick' so stats recording works correctly!
        INSERT INTO games (player1_id, player2_id, status, best_of, word_length, target_word, mixed_mode, current_turn, current_round, duration, game_mode)
        VALUES (v_opponent_id, p_user_id, 'active', 3, length(v_initial_word), v_initial_word, true, v_opponent_id, 1, 60, 'quick')
